    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?php echo action('\Tikweb\TranslationManager\Controller@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?php echo action('\Tikweb\TranslationManager\Controller@getIndex') ?>';
                }
            });
...
<header class="navbar navbar-static-top navbar-inverse" id="top" role="banner">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\Tikweb\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                Translation Manager
            </a>
        </div>
    </div>
...
<div class="container-fluid">
    ...
    <p>
        <?php if(!isset($group)) : ?>
        <form class="form-import" method="POST" action="<?php echo action('\Tikweb\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
        ...
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?php echo action('\Tikweb\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?php echo $group ?>? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >Publish translations</button>
                <a href="<?= action('\Tikweb\TranslationManager\Controller@getIndex') ?>" class="btn btn-default">Back</a>
            </form>
        <?php endif; ?>
    </p>
    <?php endif; ?>
    <form role="form" method="POST" action="<?php echo action('\Tikweb\TranslationManager\Controller@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
    ...
    <hr>
    <?php if($group): ?>
        <form class="form-find" method="POST" action="<?php echo action('\Tikweb\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
            <div class="form-group">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        ...
        <hr>
        <?php if($modifyEnabled) : ?>
        <form action="<?php echo action('\Tikweb\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
        ...
        <?php endif; ?>
        <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form" action="<?php echo action('\Tikweb\TranslationManager\Controller@postTranslateMissing') ?>">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
    ...
    <h4>Total: <?= $numTranslations ?>, changed: <?= $numChanged ?></h4>
        <form action="<?php echo action('\Tikweb\TranslationManager\Controller@getIndex', array($group)) ?>" method="GET"  role="form">
            <div class="form-group">
                <input type="text" id="search" name="search" placeholder="Enter content for search!" class="form-control" required>
            </div>
            <div class="form-group">
            ...
            <div class="tab-content" style="margin-top: 12px;">
                <div role="tabpanel" class="tab-pane <?php echo request()->has('search') || request()->has('all') || request()->has('order') || request()->has('orderBy') || empty(request()->all())  ? 'active' : '' ?>" id="home">
                    <table class="table">
                        <thead>
                        <tr>
                            <th width="15%">Key <span style="margin-left: 4px;cursor: pointer;" class="glyphicon glyphicon-sort" onclick="document.getElementById('orderForm').submit();"></span></th>
                            <form action="<?php echo action('\Tikweb\TranslationManager\Controller@getIndex', array($group)) ?>" method="GET" id="orderForm" style="display: none;">
                                <input type="hidden" name="order" value="<?php echo $order == 'asc' ? 'desc' : 'asc' ?>">
                            </form>
                            <?php foreach ($locales as $locale): ?>
                                <th><?= $locale ?> <span style="margin-left: 4px;cursor: pointer;" class="glyphicon glyphicon-sort" onclick="document.getElementById('orderBy').value = '<?php echo $orderBy == $locale.'-asc' ? $locale.'-desc' : $locale.'-asc'; ?>';document.getElementById('orderByForm').submit();"></span></th>
                                <form action="<?php echo action('\Tikweb\TranslationManager\Controller@getIndex', array($group)) ?>" method="GET" id="orderByForm" style="display: none;">
                        ...
                        <?php foreach ($translations as $key => $translation): ?>
                            <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                                <td>
                                    ...
                                    <span><button style="margin-right:3px;" type="button" class="btn btn-xs btn-warning" data-toggle="modal" data-target="#myModalPath<?php echo $i; ?>"><span class="glyphicon glyphicon-road"></span></button></span> <span style="margin-left: 2px;"><?php $data = \Tikweb\TranslationManager\Models\Translation::where('locale', config('app.locale'))->where('key', $key)->first()['filepath']; if(is_null($data)) { echo 0;} else { echo count(json_decode($data));}  ?></span>
                                    <!-- Modal -->
                                    <div class="modal fade" id="myModalPath<?php echo $i; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                        <div class="modal-dialog" role="document">
                                            ...
                                            <div class="modal-body">
                                                <?php $tra = \Tikweb\TranslationManager\Models\Translation::where('locale', config('app.locale'))->where('key', $key)->first(); ?>
                                ...
                                <?php foreach ($locales as $locale): ?>
                                    ...
                                    <td>
                                        ...
                                        <?php if ($t && $t->status == 1 && $modifyEnabled): ?>
                                                <form action="<?php echo action('\Tikweb\TranslationManager\Controller@postApprove', [$group]) ?>" method="POST" role="form">
                                                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                ...
                                <?php if ($deleteEnabled && $modifyEnabled): ?>
                                    <td>
                                        <a href="<?php echo action('\Tikweb\TranslationManager\Controller@postDelete', [$group, $key]) ?>"
                                        class="delete-key"
                            ...
                            <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                                ...
                                <?php foreach ($locales as $locale): ?>
                                    ...
                                    <td>
                                        ...
                                        <?php if ($t && $t->status == 1): ?>
                                                <form action="<?php echo action('\Tikweb\TranslationManager\Controller@postApprove', [$group, $locale, $key,]) ?>" method="POST" role="form">
                                                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                                                    <input type="hidden" name="previous" value="<?php echo url()->full(); ?>">
                            ...
                            <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                                ...
                                <?php $t = null ?>
                                    <td>
                                        <div style="display: flex;">
                                            <a href="#edit" style="margin-right: 6px;" class="editable status-<?php echo $t ? $t->status : 0 ?> locale-<?php echo $locale ?>"
                                        data-locale="<?php echo $keylocale ?>" data-name="<?php echo $keylocale . "|" . htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>"
                                        id="username" data-type="textarea" data-pk="<?php echo $t ? $t->id : 0 ?>"
                                        data-url="<?php echo $editUrl ?>"
                                        data-title="Enter translation"><?php echo $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' ?></a>
                                        </div>
    ...
    <?php else: ?>
        <fieldset>
            ...
            <form  class="form-remove-locale" method="POST" role="form" action="<?php echo action('\Tikweb\TranslationManager\Controller@postRemoveLocale') ?>" data-confirm="Are you sure to remove this locale and all of data?">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            ...
            <?php if($modifyEnabled) : ?>
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Tikweb\TranslationManager\Controller@postAddLocale') ?>">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <div class="form-group">
                    <p>
        ...
        <fieldset>
            ...
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Tikweb\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
